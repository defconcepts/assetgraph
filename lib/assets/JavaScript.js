        ...
        if (!('_text' in this)) {
            ...
            if (parseTree) {
                var outputOptions = _.defaults({}, this.serializationOptions, this.assetGraph && this.assetGraph.javaScriptSerializationOptions, {
                    ...
                    // when it's false:
                    comments: this.isPretty,
                    compact: !this.isPretty
                });
                var text = escodegen.generate(parseTree, outputOptions);
                this._text = text;
            } else {
    ...
    findOutgoingRelationsInParseTree: function () {
        ...
        var tryFoldConstantToString = function (node) {
            if (node.type === 'Literal') {
                ...
                var foldedNode = esmangle.mangle(node);
                if (foldedNode.type === 'Literal' && typeof foldedNode.value === 'string') {
                    this.markDirty();
                    return foldedNode;
                } else {
                    return node;
        ...
        estraverse.traverse(parseTree, {
            enter: function (node, parentNode) {
                var stack = this.parents(); // DOES THIS INCLUDE THE NODE ITSELF?
                ...
                if (node.type === 'CallExpression') {
                                    ...
                                    enter: function (node, parentNode) {
                                        if (node.type === 'ObjectExpression') {
                                            node.properties.forEach(function (property) {
                                                if (property.kind === 'init' && property.key.type === 'Literal') {
                                            ...
                                            if (node.body.type === 'CallExpression' &&
                                                node.body.callee.type === 'MemberExpression' && !node.body.callee.computed &&
                                                node.body.callee.property.name === 'put' &&
                                                node.body.callee.object.type === 'Identifier' &&
                                                node.body.callee.object.name === templateCacheVariableName &&
                                                node.body.arguments.length === 2 &&
                                                node.body.arguments[0].type === 'Literal' && typeof node.body.arguments[0].value === 'string' &&
                                                node.body.arguments[1].type === 'Literal' && typeof node.body.arguments[0].value === 'string') {

                                                outgoingRelations.push(new AssetGraph.JavaScriptAngularJsTemplateCacheAssignment({
                                                    from: that,
                                                    to: new AssetGraph.Html({
                                                        isExternalizable: false,
                                                        text: node.body.arguments[1].value
                                                    }),
                                                    node: node,
                    ...
                    if (node.callee.type === 'Identifier' && node.callee.name === 'INCLUDE') {
                        ...
                        OUTER:
                        while (parentIndex >= 0) {
                            if (stack[parentIndex].type === 'BlockStatement') {
...
JavaScript.uglifyJs = require('uglify-js-papandreou');
