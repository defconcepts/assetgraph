    ...
    return function replaceSymbolsInJavaScript(assetGraph) {
        assetGraph.findAssets(_.extend({type: 'JavaScript'}, queryObj)).forEach(function (javaScript) {
            ...
            estraverse.traverse(javaScript.parseTree, {
                enter: function (node) {
                    var replacementAst;
                    if (node.type === 'Identifier' && Object.prototype.hasOwnProperty.call(replacementAstBySymbolName, node.name) && !isLeftHandSideOfAssignment(this.parents())) {
                    ...
                    if (replacementAst) {
                        var stack = this.parents().reverse(); // DOES THIS INCLUDE THE NODE ITSELF?
                        ...
                        stack.some(function (node, stackIndex) {
                            ...
                            if (originalNode === node) {
                                ...
                                if (node.computed && node.property.type !== 'Literal')   {
                                    ...
                                    var propVal = node.computed ? String(node.property.value) : node.property;
